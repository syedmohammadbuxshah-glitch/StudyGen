...
import { Lock, User, Sparkles, Moon, Sun, GraduationCap, Eye, EyeOff } from "lucide-react";
...
const LOCAL_USERS_KEY = "studygen_local_users";
...
export const LoginPage: React.FC<LoginPageProps> = ({
  ...
  const finishLogin = (user: { username: string; role: string }) => {
    localStorage.setItem("studygen_auth_user", JSON.stringify(user));
    // App reads the authenticated user on startup. Reloading also avoids
    // depending on a backend API that is unavailable on static GitHub Pages.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isRegistering) {
        ...
        finishLogin({ username: cleanUsername, role: "student" });
        return;
      }

      // Static GitHub Pages login: no server/API is required.
      const demoUsers: StoredUser[] = [
        { username: "student", password: "password123", role: "student" },
        { username: "admin", password: "admin123", role: "admin" },
      ];
      const users = [...demoUsers, ...getLocalUsers()];
      ...
      if (matchedUser) {
        finishLogin({ username: matchedUser.username, role: matchedUser.role });
        return;
      }

      // If a backend is available, preserve the original API login as a fallback.
    ...
    <div className={`min-h-screen font-sans flex flex-col justify-center items-center relative overflow-hidden p-4 transition-colors duration-300 ${
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="text-center mb-8 flex flex-col items-center">
          ...
          <p className={`text-xs uppercase tracking-widest font-semibold ${darkMode ? "text-indigo-200/80" : "text-slate-500"}`}>
        ...
        <div className={`rounded-3xl border p-8 transition-all duration-300 shadow-2xl backdrop-blur-md ${
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider mb-2 opacity-80">Password</label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 flex items-center pl-3.5 opacity-60">
            ...
            {error && (
              <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-3 text-xs text-red-400 font-semibold text-center">
                ⚠️ {error}
              </div>
            )}
            ...
            >
              {loading ? <span>Loading...</span> : <><span>{isRegistering ? "Register Account" : "Sign In"}</span><Sparkles className="w-4 h-4 text-yellow-300 animate-pulse-glow" /></>}
            </button>
          </form>

          <div className="mt-6 text-center text-xs">
            <span className="opacity-60">{isRegistering ? "Already have an account? " : "New to StudyGen? "}</span>
            <button
        ...
        <div className={`mt-6 p-4 rounded-2xl border text-center text-xs space-y-1.5 ${
          darkMode ? "bg-white/5 border-white/10 text-indigo-200" : "bg-indigo-50/70 border-indigo-100 text-indigo-950"
        }`}>
          <div className="font-bold uppercase tracking-wider text-[10px] text-yellow-500 dark:text-yellow-400 mb-1">Demo Credentials</div>
          <div>👨‍🎓 Student User: <code className="bg-black/20 px-1.5 py-0.5 rounded font-mono">student</code> / <code className="bg-black/20 px-1.5 py-0.5 rounded font-mono">password123</code></div>
          <div>🛡️ Admin User: <code className="bg-black/20 px-1.5 py-0.5 rounded font-mono">admin</code> / <code className="bg-black/20 px-1.5 py-0.5 rounded font-mono">admin123</code></div>
        </div>
      </div>
    </div>
  );
